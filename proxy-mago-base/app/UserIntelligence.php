...
final class UserIntelligence
{
    ...
    public static function liveConnections(array $filters = [], int $limit = 200): array
    {
        $norm = [
            ...
            'only_streaming' => !empty($filters['only_streaming']),
            'direct' => !empty($filters['direct']),
            'include_fetch' => !empty($filters['include_fetch']),
        ];
        $limit = max(20, min(500, $limit));
        $key = 'live_connections_' . md5(json_encode([$norm, $limit], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));

        return Cache::remember($key, 2, static function () use ($norm, $limit): array {
            ...
            foreach ($rows as $r) {
                ...
                if ($norm['kind'] !== '' && $norm['kind'] !== 'all') {
                    if ($norm['kind'] === 'fetch' && $isVideo) { continue; }
                    if ($norm['kind'] !== 'fetch' && (!$isVideo || $ck !== $norm['kind'])) { continue; }
                }
                if ($norm['only_streaming'] && !$streaming) { continue; }
                if ($norm['direct'] && $r['delivery_effective'] !== 'direct_source') { continue; }
                if (!$norm['include_fetch'] && !$isVideo && ($norm['kind'] === '' || $norm['kind'] === 'all')) { continue; }
                $out[] = $r;
            }
