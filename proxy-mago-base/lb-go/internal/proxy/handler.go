...
func (h *Handler) deliver(
	...
	for {
		...
		// Direct source: 3xx para um CDN de terceiros é seguido POR DENTRO, o
		// host novo entra no mascaramento e o cliente nunca o vê.
		if loc := resp.Header.Get("Location"); loc != "" && resp.StatusCode >= 300 && resp.StatusCode < 400 {
			resp.Body.Close()
			if !snap.Runtime.FollowDirectSource || hops >= h.MaxHops {
			...
			hops++
			if !strings.EqualFold(next.Hostname(), o.Host) {
				directHost = strings.ToLower(next.Hostname())
